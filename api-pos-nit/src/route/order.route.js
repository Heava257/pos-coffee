...
module.exports = (app) => {
    ...
    app.get("/api/order-web/active", require("../controller/order.controller").getActiveOrderByTable); // Get active bill for table
    app.get("/api/order-web/:order_id", getOrderDetail); // Status tracking for guests
    app.get("/api/order-web/customer/:customer_id", getList); // History for portal
