...
exports.create = async (req, res) => {
    try {
        ...
        const [data] = await db.query(
            "INSERT INTO expense (business_id, branch_id, expense_type_id, amount, description, expense_date) VALUES (?, ?, ?, ?, ?, ?)",
            [business_id, branch_id, expense_type_id, amount, description, expense_date]
        );
...
exports.update = async (req, res) => {
    try {
        const { business_id } = req;
        const { id, expense_type_id, amount, description, expense_date } = req.body;
        await db.query(
            "UPDATE expense SET expense_type_id=?, amount=?, description=?, expense_date=? WHERE id=? AND business_id=?",
            [expense_type_id, amount, description, expense_date, id, business_id]
        );
