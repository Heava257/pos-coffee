...
// 9. Get Bulk Sizes & Addons for a list of products
exports.getBulkConfig = async (req, res) => {
    try {
        const { product_ids } = req.body;
        if (!product_ids || !Array.isArray(product_ids) || product_ids.length === 0) {
            return res.json({ success: true, sizes: {}, addons: {} });
        }

        // Fetch sizes (variations)
        const [sizesRows] = await db.query(
            "SELECT product_id, id, label as name, price FROM product_variation WHERE product_id IN (?)",
            [product_ids]
        );

        // Fetch addons
        const [addonsRows] = await db.query(
            "SELECT product_id, id, label as name, price FROM product_addon WHERE product_id IN (?)",
            [product_ids]
        );
        ...
        sizesRows.forEach(row => {
            if (!sizes[row.product_id]) sizes[row.product_id] = [];
            sizes[row.product_id].push({
                id: row.id,
                name: row.name,
        ...
        addonsRows.forEach(row => {
            if (!addons[row.product_id]) addons[row.product_id] = [];
            addons[row.product_id].push({
                id: row.id,
                name: row.name,
                value: row.id.toString(),
                price: parseFloat(row.price || 0)
        ...
        logError("product.getBulkConfig", error, res);
    }
};
