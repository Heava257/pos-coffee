const { db, logError, removeFile, checkPlanLimit } = require("../../src/util/helper");
...
exports.getList = async (req, res) => {
    try {
        ...
        LEFT JOIN business_categories bc ON c.id = bc.category_id AND bc.business_id = p.business_id
        WHERE p.business_id = ? AND (p.business_id = 1 OR (bc.is_active = 1 AND bc.business_id = p.business_id))
    `;
...
exports.create = async (req, res) => {
    ...
    try {
        ...
        const image = req.file ? (req.file.path?.startsWith("http") ? req.file.path : req.file.filename) : null;
...
exports.update = async (req, res) => {
    ...
    try {
        ...
        const image = req.file ? (req.file.path?.startsWith("http") ? req.file.path : req.file.filename) : undefined;
