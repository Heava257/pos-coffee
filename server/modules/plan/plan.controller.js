...
exports.updatePlan = async (req, res) => {
    try {
        const { id, name, max_branches, max_staff, max_products, max_categories, price, billing_cycle, is_active, active_modules } = req.body;
        await db.query(`
            UPDATE subscription_plans SET 
                ...
                active_modules = ?
            WHERE id = ?
        `, [name, max_branches, max_staff, max_products, max_categories || 10, price, billing_cycle || 'monthly', is_active, active_modules, id]);
...
exports.createPlan = async (req, res) => {
    try {
        ...
        const [newRows] = await db.query("SELECT * FROM subscription_plans WHERE id = ?", [result.insertId]);
