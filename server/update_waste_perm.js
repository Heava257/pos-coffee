...
const update = async () => {
  try {
    // 1. Add permission
    const [exists] = await db.query("SELECT id FROM permissions WHERE route_key = 'waste'");
    let pid;
    if (exists.length === 0) {
      const [res] = await db.query("INSERT INTO permissions (name, route_key, min_plan_id) VALUES ('Waste Tracking', 'waste', 1)");
      pid = res.insertId;
      console.log("Added waste permission");
    } else {
      pid = exists[0].id;
    }
    ...
    for (const rid of roles.map(r => r.id)) {
      ...
      if (has.length === 0) {
        ...
        console.log(`Granted waste to role ${rid}`);
      }
    }
  } catch (err) {
    console.error(err);
