    ...
    try {
        ...
        if (recipe.length > 0) {
            for (const ingredient of recipe) {
                ...
                const [rmCheck] = await conn.query("SELECT name, qty, min_stock FROM raw_material WHERE id = ?", [ingredient.raw_material_id]);
                if (rmCheck.length > 0) {
                    const { name: rmName, qty: currentQty, min_stock: minStock } = rmCheck[0];
                    if (currentQty <= minStock) {
                        const [exist] = await conn.query(
                            "SELECT id FROM system_notifications WHERE business_id = ? AND type = 'inventory' AND message LIKE ? AND created_at >= NOW() - INTERVAL 12 HOUR",
                            [business_id, `%${rmName}%`]
                        );
                        if (exist.length === 0) {
                            await conn.query(
                                "INSERT INTO system_notifications (business_id, title, message, type, is_read) VALUES (?, ?, ?, 'inventory', 0)",
                                [
                                    business_id,
                                    "Low Ingredient Alert / គ្រឿងផ្សំស្តុកទាប",
                                    `Ingredient "${rmName}" is running low. Current stock: ${Number(currentQty).toFixed(2)}, below minimum limit of ${minStock}.`
                                ]
                            );
                        }
                    }
            ...
            // Check for Product Low Stock Notification
            const [bpCheck] = await conn.query(
                "SELECT bp.stock_qty, bp.min_stock_alert, p.name FROM branch_products bp JOIN products p ON bp.product_id = p.id WHERE bp.product_id = ? AND bp.branch_id = ?",
                [productId, branch_id]
            );
            if (bpCheck.length > 0) {
                const { stock_qty: currentQty, min_stock_alert: minStockAlert, name: prodName } = bpCheck[0];
                if (minStockAlert !== null && currentQty <= minStockAlert) {
                    const [exist] = await conn.query(
                        "SELECT id FROM system_notifications WHERE business_id = ? AND type = 'inventory' AND message LIKE ? AND created_at >= NOW() - INTERVAL 12 HOUR",
                        [business_id, `%${prodName}%`]
                    );
                    if (exist.length === 0) {
                        await conn.query(
                            "INSERT INTO system_notifications (business_id, title, message, type, is_read) VALUES (?, ?, ?, 'inventory', 0)",
                            [
                                business_id,
                                "Low Product Stock / ផលិតផលស្តុកទាប",
                                `Product "${prodName}" is running low in branch. Current stock: ${currentQty}, below minimum limit of ${minStockAlert}.`
                            ]
                        );
                    }
                }
        ...
        for (const item of cart_items) {
            ...
            const deductStock = async (productId, qtyMultiplier, itemName, sizeLabel = null) => {
                ...
                if (recipe.length > 0) {
                    ...
                    // Check if product stock is below min_stock_alert
                    const [bpCheck] = await conn.query(
                        "SELECT bp.stock_qty, bp.min_stock_alert, p.name FROM branch_products bp JOIN products p ON bp.product_id = p.id WHERE bp.product_id = ? AND bp.branch_id = ?",
                        [productId, branch_id]
                    );
