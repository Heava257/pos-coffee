...
const update = async () => {
  try {
    // 1. Add permission
    const [exists] = await db.query("SELECT id FROM permissions WHERE route_key = 'inventory/forecast'");
    let pid;
    if (exists.length === 0) {
      const [res] = await db.query("INSERT INTO permissions (name, route_key, min_plan_id) VALUES ('AI Forecasting', 'inventory/forecast', 1)");
      pid = res.insertId;
      console.log("Added forecast permission");
    } else {
      pid = exists[0].id;
    }
    ...
    for (const rid of roles.map(r => r.id)) {
      ...
      if (has.length === 0) {
        ...
        console.log(`Granted forecast to role ${rid}`);
      }
    }
  } catch (err) {
    console.error(err);
